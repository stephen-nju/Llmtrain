...
export reward_model=http://localhost:8081
...
##reward model
export reward_model_type=api

#ppo
export ppo_score_norm=false
export ppo_whiten_rewards=false
export ppo_target=6
export ppo_init_kl_coef=0.2
export ppo_adap_kl_ctrl=true
export ppo_gamma=1
export ppo_lam=0.95
export ppo_cliprange=0.2
export ppo_cliprange_value=0.2
## eval
export eval_dataset
export eval_steps
...
pref_loss:,pref_beta:,simpo_gamma:,ddp_timeout:,neftune_noise_alpha:,max_new_tokens:,top_p:,top_k:,ref_model_adapters:,\
lora_rank:,lora_target:,lora_alpha:,loraplus_lr_embedding:,loraplus_lr_ratio:,reward_model_type:,reward_model:,\
ppo_score_norm:,ppo_whiten_rewards:,ppo_target:,ppo_init_kl_coef:,ppo_adap_kl_ctrl:,ppo_gamma:,ppo_lam:,ppo_cliprange:,ppo_cliprange_value:,\
...
if [[ -n ${reward_model} ]]; then
	optional_params+=(--reward_model ${reward_model})
fi

if [[ -n ${ref_model_adapters} ]]; then
	optional_params+=(--ref_model_adapters ${ref_model_adapters})
fi
